...
?- findall( [P,P],(pp([P,P])), D ), scf( arrow-_, F, D, Z), fig( arrow-_, F ), fail.
...
?- findall( [P,P],(pp([P,P])), D ), check_scf( gs-_, D, Z).
D = [[[a, b, c], [a, b, c]], [[a, c, b], [a, c, b]], [[b, a, c], [b, a, c]], [[b, c, a], [b, c, a]], [[c, a, b], [c, a|...]], [[c, b|...], [c|...]]],
Z = poss.

?- findall( [P,P],(pp([P,P])), D ), scf( arrow-_, F, D, Z), fig( arrow-_, F ).
...
rdict_triple( T, D, D, Z ):-
 	 check_scf( T, D, Z ).
rdict_triple( T, D, A, Z ):-
	 sd_triple( X, _ ),
	 \+ subset( X, A ),
	 union( X, A, Dx ),
	 %Z = dict,
	 rdict_triple( T, D, Dx, Z ).


pp_spread_decisive_0([[ X, Y, Z ],[ Y, X, Z ]], [[X, Y, Z], [ Y, Z, X]]).
pp_spread_decisive_0([[ X, Y, Z ],[ Y, X, Z ]], [[X, Z, Y], [ Y, X, Z]]).
pp_spread_decisive( P, Q ):- pp_spread_decisive_0( P, Q ).
pp_spread_decisive( P, Q ):- pp_spread_decisive_0( Q, P ).

sd_triple( [ P, A, B ], Z ):-
	pp( P ), 
	bagof( Q,( pp_spread_decisive( P, Q )), [A,B ] ), 	check_scf( arrow-_, [P,A,B], Z ).
...
?- X=arrow-type, hist((rdict_triple( X, D, [], Z ), length( D, N )), N:Z ).
...
?- X=gs-type, hist((rdict_triple( X, D, [], Z ), length( D, N )), N:Z ).
 (0:poss):1
 (3:dict):6
 (6:dict):12
 (6:poss):18
...
?- rdict_triple( T, D, [], Z ), length( D, N ), N =18, fig( membership, D ).
...
6:[c,b,a]  -  a2 a1 g1 -  -  
--
D = [[[c, b, a], [b, c, a]], [[c, b, a], [b, a, c]], [[c, a, b], [b, c, a]], [[c, a, b], [a, c, b]], [[c, a, b], [a, b|...]], [[c, b|...], [a|...]], [[b|...], [...|...]], [[...|...]|...], [...|...]|...],
Z = poss,
N = 18 .

?- X=arrow-type, findall(S, (rdict_triple( X, D, [], Z ), Z=dict, length( D, N ), N=9, sort(D,S) ), L ), sort( L, H ), member( D, H ), fig( membership, D ), fail.
...
?- X=arrow-type, findall(S, (rdict_triple( X, D, [], Z ), Z=dict, length( D, N ), N=12, sort(D,S) ), L ), sort( L, H ), member( D, H ), fig( membership, D ), fail.
...
?- X=arrow-type, findall(S, (rdict_triple( X, D, [], Z ), Z=dict, length( D, N ), N=15, sort(D,S) ), L ), sort( L, H ), member( D, H ), fig( membership, D ), fail.
...
?- hist((pp(P), bagof( Q,( pp_spread_decisive( P, Q )), [A,B ] ), pp( C ), check_scf( arrow-_, [P,A,B,C], Z ) ), Z ).
...
?- pp(P), bagof( Q,( pp_spread_decisive( P, Q )), [A,B ] ), pp( C ), check_scf( arrow-_, [P,A,B,C], Z ), Z=dict, fig( membership, [P,A,B,C] ), fail.
...
:- dynamic tmp_rdict/4.
init_rdict:- abolish( tmp_rdict/4 ).

rdict( _, D, D, N ):- length( D, N ).
rdict( T, D, A, N ):-
	 pp( X ),
	 \+ member( X, A ),
	 %arrow_ring( both, Ra ),
	 %member( X, Ra ),
	 Dx = [ X | A ],
	 \+ check_recurrent_dom( T, X, Dx, _ ),
	 check_scf( T, Dx, Z ),
	 Z = dict,
	 length( Dx, K ), 
	 assert( tmp_rdict( T, X, Dx, K ) ),
	 rdict( T, D, Dx, N ).
...
check_recurrent_dom( T, P, D, Dx ):-
	 clause( tmp_rdict( T, P, Dx, _ ), _ ),
	 subset( D, Dx ).
...
?- pp( P ), rdict_from( arrow-type, P, D, K ), K>20, !, fail.
false.

?- hist( tmp_rdict( T, P, D, K ), K ).
...
?- tmp_rdict( T, P, D, K ),K=12, fig( domain, D  ), write(K),fail.
...
?- hist((arrow_ring( A, D ),!, select_n( X, D, K ), check_scf( arrow-type, X, Z ) ), Z:K ).
...
?- hist((arrow_ring( both, D ), select_n( X, D, K ), check_scf( arrow-type, X, Z ) ), Z:K ).
...
?- pp( P ), rdict_from( arrow-type, P, D, K ), K>35, !, fail.
false.

?- hist((pp(P), bagof( Q,( pp_spread_decisive( P, Q )), [A,B ] ), pp( C ), check_scf( arrow-_, [P,A,B,C], Z ), Z=dict,  rdict( arrow-type, D, [P,A,B,C], K )), K ).
...
% SWISH version of this program is available: https://swish.swi-prolog.org/p/HaEfmQok.pl
...
scf( Type, F, D, Z ):-
	 ...
	 ( sort( L, [J] ) -> W=dict(J); W=poss ),
	 Z = W.
%
*/
/*
?- hist((arrow_ring( A, D ), scf( gs-type, F, D,Z)), Z ).
...
total:2457
true.
?- hist((pareto_ring( A, D ), scf( arrow-type, F, D,Z)), Z ).
...
?- hist((pareto_ring( A, D ), scf( gs-type, F, D,Z)), Z ).
...
ルール解説(R:- G):- ルール(R:- G, _, _, [H,W,T,A] ), nl, write( H), writeln(W),\+  ( member( X, T ), \+ writeln( X ) ), write( A ).

/*

マニピュラビリティ(C.4)の最初のルールは次のように説明される．

?- ルール解説(manipulable(_,_,_,_):- G).

マニピュラブル(1,[A,B]-C,[D,B]-E,F)は
[[D,B]-E,は,F,の要素である]
かつ
[[順序,A,において,E,は,C,よりも好ましい],または,[順序,D,において,C,は,E,よりも好ましい]]
という条件をすべて満たすときは正しい
G = (member([D, B]-E, F), (p(E, C, A);p(C, E, D))).

ノンインポーズド条件（C.5）については次のように翻案される．

?- ルール解説( non_imposed(_):-G).
